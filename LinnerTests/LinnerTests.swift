...
import XCTest
@testable import Linner

class LinnerTests: XCTestCase {
    ...
    func setupGivenFor(modelType: ModelType) {
        ...
        switch modelType {
        case .menus :
            menusViewModelUnderTest = MenusViewModel(with: apiClientUnderTest!)
            setupMockSession(for: menusViewModelUnderTest!)
        case .recipes:
            recipesViewModelUnderTest = RecipesViewModel(with: apiClientUnderTest!, menu: Menu(id: 1, title: "Lunch"))
